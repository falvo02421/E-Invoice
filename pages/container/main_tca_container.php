...
$(document).ready(function() {
	...
	function(data) {
		...
		else if(obj.rc == 'S' && obj.data.info == 'OK') {
		...
		else if(obj.rc == 'S' && obj.data.info == 'OKTCA'){
			
			var txt;
			if (confirm("TRUCK ID SUDAH DIDAFTARKAN SEBELUMNYA,JIKA ANDA INGIN TRUCK IN ANDA HARUS MEMBAWA COMBO CONTAINER INI DENGAN CONTAINER SEBELUMNYA,PILIH OK JIKA SETUJU")) {
				var url="<?=ROOT?>truck_container/save_tca_association";
				$.post(url,{terminal:terminal,eservice:eservice,no_request:no_request,no_container:no_container,truck_id:truck_id,pin_number:pin_number,'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'}
				);
				window.location = "<?=ROOT?>truck_container/main_tca";
							...
							<div class="main-box">
								...
								<div class="form-group">
									...
									<div class="form-group col-xs-3">
										...
										<input type="text" class="form-control" id="truck_id" name="truck_id" placeholder="Truck ID" title="Masukkan data customer" size="8">
									</div>
									<div class="form-group col-xs-3">
